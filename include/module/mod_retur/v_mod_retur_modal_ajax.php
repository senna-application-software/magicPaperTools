...
switch($jenis_aksi)
{
	...
	case 'retur_edit':	
	$nosrt    = $generates->query($q="SELECT * FROM t_retur WHERE id_surat = '$parameter' ");
  $no_srt   = $nosrt->fetchAll();
        ...
        <div class='form-group'>
              ...
              <option value='Driver Kami'>DRIVER VICCI</option>
              <option value='____________'>OTHER</option>
				...
				<div class='modal-footer'>
					<button type='submit' name='create' id='next_retur_edit'   modact='../modact/retur.avx'  class='btn btn-primary'>Save</button>
  ...
  $nosrt    = $generates->query($q='SELECT MAX(id_surat) AS maxim FROM t_retur');
  $no_srt   = $nosrt->fetchAll();
  $no_baru  = $no_srt[0]['maxim'] + 1;
  $no_barus = strlen($no_baru);
  $kd_baru  = str_repeat("0",7 - $no_barus).$no_baru;	
  $no_surat = $kd_baru . "/MSO-DIV/TRK/" . $roman . "/" . date("Y"); 
		echo "
			<div class='form-horizontal'>		
        ...
        <div class='form-group'>
              ...
              <option value='____________'>OTHER</option>
				...
				<div class='modal-footer'>
					<button type='submit' name='create' id='next_retur_baru'   modact='../modact/retur.avx'  class='btn btn-primary'>Save</button>
					<button type='button' class='btn btn-default' data-dismiss='modal'>Close</button>
				</div>			
			</div>
		";	
...
/**
* End of file v_mod_retur_modal_ajax.php
* Location: ../mod_retur/v_mod_retur_modal_ajax.php
*/